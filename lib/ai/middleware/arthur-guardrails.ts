...
export function createArthurGuardrailsMiddleware(
  ...
  return {
    wrapGenerate: async ({ doGenerate, params }) => {
      ...
      console.debug('[arthur-guardrails] wrapGenerate - messages:', messages);
      console.debug(
        '[arthur-guardrails] wrapGenerate - lastUserMessage:',
        lastUserMessage,
      );
      ...
      try {
        const providerMetadata = params.providerMetadata as
          ...
          | undefined;
        console.debug(
          '[arthur-guardrails] wrapGenerate - calling validatePrompt with:',
          {
            taskId,
            prompt: textContent,
            conversation_id: providerMetadata?.conversationId,
            user_id: providerMetadata?.userId,
          },
        );
    ...
    wrapStream: async ({ doStream, params }) => {
      ...
      console.debug('[arthur-guardrails] wrapStream - messages:', messages);
      console.debug(
        '[arthur-guardrails] wrapStream - lastUserMessage:',
        lastUserMessage,
      );
      ...
      try {
        const providerMetadata = params.providerMetadata as
          ...
          | undefined;
        console.debug(
          '[arthur-guardrails] wrapStream - calling validatePrompt with:',
          {
            taskId,
            prompt: textContent,
            conversation_id: providerMetadata?.conversationId,
